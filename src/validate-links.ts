...
export default async function main({ dest, file }: Input) {
	...
	const useGet = yesOrNo(env("INPUTS_VALIDATE_LINKS_USE_GET")) ? "--http-always-get" : "";
	const ignoreUrls = env("INPUTS_VALIDATE_LINKS_IGNORE_URLS") ? env("INPUTS_VALIDATE_LINKS_IGNORE_URLS").split(" ") : [];
